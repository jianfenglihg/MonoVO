...
int main(int argc, char **argv)
{
    ...
    slam::Simplevo::Ptr vo ( new slam::Simplevo );
    ...
    for(int ni=0; ni<nImages; ni++)
    {
        ...
        if ( vo->vo_state_ == slam::Simplevo::LOST )
            break;
        //SE3 Tcw = pFrame->T_c_w_.inverse();
        Mat t;
        if(ni > 2)
        {
            vo->relative_t_.copyTo(t);
            std::cout<<t.at<float>(0,0)<<" ,  "<<t.at<float>(1,0)<<std::endl;
        }
        
        //Mat t = vo->relative_t_;
        ...
        cv::Point p(t.at<float>(0,0)+290, t.at<float>(1,0)+90);
